...
description: Agent skills for working with Minions Outreach MinionTypes. Provides CRUD operations, CLI usage, and best practices for AI agents managing minions-outreach data.
---

# Minions Outreach Agent Skills

Skills for agents operating on the `minions-outreach` toolbox.
...
The `outreach` CLI provides basic project info and utilities:
...
```ts
import { customTypes } from '@minions-outreach/sdk/schemas';
...
```python
from minions_outreach.schemas import custom_types
...
- This agent only writes to `minions-outreach` — it reads from other toolboxes but never writes to them
